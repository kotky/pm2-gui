...
Monitor.available = function (options) {
  ...
  var wrapLocal = function () {
    return {
      value: (options.agent && options.agent.authorization ? options.agent.authorization + '@' : '') + '0.0.0.0:' + options.port,
      short: 'localhost'
    }
  }
  if (!remotable) {
...
Monitor.toConnectionString = function (connection) {
  var uri = (connection.protocol || 'http:') + '//' + (connection.hostname || '0.0.0.0') + ':' + connection.port +
